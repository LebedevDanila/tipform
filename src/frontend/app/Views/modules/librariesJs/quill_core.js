        ...
        $(this.domCaption).keydown((e) =>
        {
            ...
            if (key == Keyboard.keys.ENTER)
            {
                ...
                if (pos.start != pos.end)
                {
                    ...
                    $target.val(value).selection('setPos',
                    {
                        start: value.length,
                        end: value.length
                    });
...
function getFigureValueByUrl(url)
{
    ...
    {
        return {
            embed: '/blog/embed/youtube?url=' + encodeURIComponent(url)
        };
    }
    if (match = url.match(/^(https?):\/\/(www\.)?vimeo\.com\/(\d+)/i))
    {
        return {
            embed: '/blog/embed/vimeo?url=' + encodeURIComponent(url)
        };
    }
    if (match = url.match(/^(https?):\/\/(www\.|mobile\.)?twitter\.com\/(.+)\/status\/(\d+)/i))
    {
        return {
            embed: '/blog/embed/twitter?url=' + encodeURIComponent(url)
        };
    }
    if (match = url.match(/^(https?):\/\/(t\.me|telegram\.me|telegram\.dog)\/([a-zA-Z0-9_]+)\/(\d+)/i))
    {
        return {
            embed: '/blog/embed/telegram?url=' + encodeURIComponent(url)
        };
    }
    if (match = url.match(/^data:(image\/gif|image\/jpe?g|image\/png|video\/mp4);base64,(.*)$/))
    {
...
function savePage()
{
    ...
    let boundary = '---------------------------DominfoBoundary21',
        body = '--' + boundary + '\r\n' +
...
function checkAuth()
{
    var data = JSON.parse('{"short_name":"","author_name":"","author_url":"","save_hash":"","can_edit":false}');
    if (data.save_hash)
    {
...
function showLinkTooltip(link, value)
{
    ...
    $tl_link_tooltip.text(decodeURI(value));
...
function tooltipUpdatePosition($tooltip, range, options)
{
    ...
    $tooltip.css(
    {
        left: tt.left,
        top: tt.top
    }).toggleClass('bottom', bottom_class);
}
...
new Image().src = window.devicePixelRatio >= 2 ? 'https://telegra.ph/images/icons_2x.png?1' : 'https://telegra.ph/images/icons.png?1'; 
